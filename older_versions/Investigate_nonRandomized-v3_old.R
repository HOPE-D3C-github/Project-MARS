...
cw_block_0_starttime <- matched_2_dec_pts_v2 %>% group_by(mars_id, study_day_int) %>% filter(!is.na(block0_start_mountain_calculated)) %>% filter(row_number() == 1) %>% 
  select(mars_id, study_day_int, olson, block0_start_mountain_calculated_cw = block0_start_mountain_calculated)
...
# Carry through participant level timezone (olson) for each participant
cw_date_dayint <- cw_date_dayint %>% group_by(mars_id) %>% fill(olson, .direction = "downup") %>% ungroup()
...
cw_date_dayint_v2 <- cw_date_dayint %>% 
  left_join(y = join_nodaystart_w_setwakeuptimes %>% select(-time_set_hrts),
            by = join_by(mars_id, v1_date_began_mountain_cw, v1_date_began_plus_nine_mountain_cw, study_day_int, study_date, olson, block0_start_mountain_calculated_cw, incorrect_date_on_block0_calculated))
...
cw_date_dayint_v3 <- cw_date_dayint_v2 %>% 
  mutate(default_wakeup_time_hrts_mtn = case_when(
    is.na(block0_start_mountain_calculated_cw) & is.na(set_wakeup_time_hrts) & olson == "US/Central" ~ (as_datetime(paste(study_date, "00:08:00"), tz = "America/Denver") - hours(1)),
    is.na(block0_start_mountain_calculated_cw) & is.na(set_wakeup_time_hrts) & olson == "US/Mountain" ~ as_datetime(paste(study_date, "00:08:00"), tz = "America/Denver"),
    is.na(block0_start_mountain_calculated_cw) & is.na(set_wakeup_time_hrts) & olson == "US/Pacific" ~ as_datetime(paste(study_date, "01:08:00"), tz = "America/Denver"),
    T ~ NA_POSIXct_
  )) %>% select(-olson)
...
systemlog_emi_blocks <- full_join(x = system_log_EMI_rands %>% mutate(in_syslog = T),
                                  ...
                                  by = join_by(mars_id, between("system_log_emi_rand_ts_mtn", block_start_hrts, block_end_hrts) ))
...
systemlog_emi_blocks %>% group_by(mars_id) %>% 
  ...
  filter(in_blocks_cw | between(system_log_emi_rand_ts_mtn, min(block_start_hrts, na.rm = T), max(block_end_hrts, na.rm = T))) %>% 
  ungroup() %>% 
...
matched_2_dec_pts_v11 <- matched_2_dec_pts_v10 %>% 
  select(all_of(colnames(matched_2_dec_pts)), study_day_int, study_date, block0_RECALCULATED_start_mountain, block_start_hrts, block_end_hrts, block_start_valid, block_end_valid,
         battery_status, battery_status_simple, A_emi_report, last_condition_emi_report, A_system_log, after_withdraw_date)



matched_2_dec_pts_v11 %>% select(mars_id, study_date, cluster_id, study_day_int, decision_point, block_number, block0_RECALCULATED_start_mountain, block_start_hrts, block_end_hrts, 
...
matched_2_dec_pts_v11 %>% filter(is.na(A)) %>% count(A, after_withdraw_date, invalid_block_start = !block_start_valid, invalid_block_end = !block_end_valid, battery_status_simple, emi_report_missing_rand =is.na(A_emi_report), emi_conditions_clear =last_condition_emi_report, as_date(block_end_hrts) != study_date) %>% 
  arrange(A, desc(after_withdraw_date), desc(invalid_block_start), desc(invalid_block_end), battery_status_simple, desc(emi_report_missing_rand), desc(emi_conditions_clear)) %>% 
  mutate(prop = n/sum(n))

# as_date(block_end_hrts) != study_date
###
if(F){
