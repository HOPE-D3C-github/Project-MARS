...
path_to_output_data <- "C:/Users/u1330076/Box/MARS metadata support for Jamie/Output"
...
pt_withdrawn_date <- readRDS(file = file.path(path_to_staged_data, "pt_withdrawn_date.RDS"))
...
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
# Testing calculating block start times using Jamie's method
# 
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
# Merge dat_wakeup filtered with study date cw
#
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
cw_date_dayint_v5 <- cw_date_dayint_v4 %>% 
  mutate(study_day_wakeup_time = coalesce(ts_wakeup_mountain, set_wakeup_time_hrts_mtn, wakeup_default_calc),
         block0_RECALCULATED_start_mountain = study_day_wakeup_time + minutes(30)) %>% 
...
cw_pt_date_blocks <- matched_2_dec_pts_v5 %>% 
  select(mars_id, study_date, study_day_int, block_number, block_start_hrts_mtn, block_end_hrts_mtn, A)

cw_pt_date_blocks %>% count(is.na(block_start_hrts_mtn), is.na(A))
...
emi_w_blocks <- full_join(x = emi_report %>% rename(mars_id = participant_id) %>% mutate(in_emi_rpt = T),
                             ...
                             by = join_by(mars_id, between(human_readable_time_try, block_start_hrts_mtn, block_end_hrts_mtn)))
...
matched_2_dec_pts_v6 <- matched_2_dec_pts_v5 %>% mutate(start_block_date = as_date(block_start_hrts_mtn),
                                end_block_date = as_date(block_end_hrts_mtn),
...
if(F){matched_2_dec_pts_v6 %>% filter(is.na(block_start_hrts_mtn)) %>% View} # 78 have no day start data. can check log for any data recorded on those days
...
battery_w_blocks <- right_join(x = battery_binned_all, y = cw_pt_date_blocks,
            by = join_by(mars_id, overlaps(battery_range_start_hrts_mtn, battery_range_end_hrts_mtn, block_start_hrts_mtn, block_end_hrts_mtn)))
...
battery_w_blocks_v2 <- battery_w_blocks %>% 
  rowwise() %>% 
  mutate(
    total_block_time = difftime(block_end_hrts_mtn, block_start_hrts_mtn, units = "mins"),
    time_battery_status_in_block = case_when(
      !is.na(block_start_hrts_mtn) ~ difftime( min(battery_range_end_hrts_mtn, block_end_hrts_mtn) , max(battery_range_start_hrts_mtn, block_start_hrts_mtn), units = "mins"),
      T ~ NA)) %>% 
  ungroup() %>% 
...
matched_2_dec_pts_v7 <- matched_2_dec_pts_v6 %>% 
  full_join(y = battery_w_blocks_wide,
            by = join_by(mars_id, study_date, study_day_int, block_number, block_start_hrts_mtn, block_end_hrts_mtn, A)) %>% 
...
systemlog_emi_blocks <- full_join(x = system_log_EMI_rands %>% mutate(in_syslog = T),
                                  ...
                                  by = join_by(mars_id, between("system_log_emi_rand_ts_mtn", block_start_hrts_mtn, block_end_hrts_mtn) ))
...
systemlog_emi_blocks %>% group_by(mars_id) %>% 
  ...
  filter(in_blocks_cw | between(system_log_emi_rand_ts_mtn, min(block_start_hrts_mtn, na.rm = T), max(block_end_hrts_mtn, na.rm = T))) %>% 
  ungroup() %>% 
...
system_log_joined_2_blocks <- system_log_v2 %>% 
  right_join(y = cw_pt_date_blocks,
             by = join_by(mars_id, between(x$system_log_hrts_mtn, y$block_start_hrts_mtn, y$block_end_hrts_mtn))) 

block_system_log <-  system_log_joined_2_blocks %>% 
  group_by(mars_id, study_date, study_day_int, block_number, block_start_hrts_mtn, block_end_hrts_mtn, A) %>% 
...
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
# STEP 3. Process for Final Summary ----
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
matched_2_dec_pts_v9 <- matched_2_dec_pts_v8 %>% 
...
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
# Collapse the information from each data stream into a limited number of key indicators to summarize the information
#   - Battery: 
...
# matched_2_dec_pts_v10$battery_status_simple <- factor(matched_2_dec_pts_v10$battery_status_simple, levels = c("No Missing Data" ~ "No Missing Data", "Some Missing Data", "No Data - Entire Block"))


saveRDS(matched_2_dec_pts_v10,
        file = file.path(path_to_staged_data, "matched_2_dec_pts_v10.RDS"))

matched_2_dec_pts_v11 <- matched_2_dec_pts_v10 %>% 
  select(all_of(colnames(matched_2_dec_pts)), study_day_int, study_date, block0_RECALCULATED_start_mountain, block_start_hrts_mtn, block_end_hrts_mtn, block_start_valid, block_end_valid,
         battery_status, battery_status_simple, A_emi_report, last_condition_emi_report, A_system_log, after_withdraw_date)

if(F){
  # saving data for E3C team to look into periods without any battery data
  saveRDS(matched_2_dec_pts_v11, file = file.path(path_to_output_data, "matched_2_dec_pts_w_CFH_metadata_20230612.RDS"))
  saveRDS(battery_binned_all, file = file.path(path_to_output_data, "MARS_battery_binned_all_20230612.RDS"))
}

matched_2_dec_pts_v11 %>% select(mars_id, study_date, cluster_id, study_day_int, decision_point, block_number, block0_RECALCULATED_start_mountain, block_start_hrts_mtn, block_end_hrts_mtn, 
...
matched_2_dec_pts_v11 %>% filter(is.na(A)) %>% count(A, A_emi_report, last_condition_emi_report, A_system_log, battery_status, block_start_valid, block_end_valid, is_wakeup_default, after_withdraw_date) %>% View

matched_2_dec_pts_v11 %>% filter(is.na(A)) %>% count(A, after_withdraw_date, invalid_block_start = !block_start_valid, invalid_block_end = !block_end_valid, battery_status_simple, emi_report_missing_rand =is.na(A_emi_report), emi_conditions_clear =last_condition_emi_report) %>% 
...
matched_2_dec_pts_v11 %>% filter(is.na(A)) %>% count(A, after_withdraw_date, invalid_block_start = !block_start_valid, invalid_block_end = !block_end_valid, battery_status_simple, emi_report_missing_rand =is.na(A_emi_report), emi_conditions_clear =last_condition_emi_report, as_date(block_end_hrts_mtn) != study_date) %>% 
  arrange(A, desc(after_withdraw_date), desc(invalid_block_start), desc(invalid_block_end), battery_status_simple, desc(emi_report_missing_rand), desc(emi_conditions_clear)) %>% 
  mutate(prop = n/sum(n))

# as_date(block_end_hrts_mtn) != study_date
###
if(F){
  matched_2_dec_pts_v11 %>% count(battery_status, A) %>% group_by(battery_status) %>% mutate(percent_by_battery_status = scales::percent(n/sum(n))) %>% View
  matched_2_dec_pts_v11 %>% count(battery_status, A) %>% group_by(battery_status) %>% mutate(percent_by_battery_status_not_all_shown = scales::percent(n/sum(n))) %>% filter(is.na(A)) 
  
  matched_2_dec_pts_v11 %>% filter(is.na(A) & battery_status == 'No Missing Data') %>% count(study_day_int)
  matched_2_dec_pts_v11 %>% filter(is.na(A) & battery_status == 'No Data - Entire Block') %>% count(study_day_int)
